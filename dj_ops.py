...
class ProcessEntriesHandler(ABC):
    """ Mixin Class which provides a basic implementation of 
        ...
        Duplicates are not removed; if required use the `iset_unique` 
...
class ISetIfAny(Operation):
    ...
    def op_name() -> str: return "iset_if_any"
    ...
    def __str__(self):
        ...
        return f"{ISetIfAny.op_name()}({config}{self.cop})"
...
class ISetIfAll(Operation):
    ...
    def op_name() -> str: return "iset_if_all"
    ...
    def __str__(self):
        ...
        if self.on_none or self.on_empty:
            config = f"N/A={self.on_none}, []={self.on_empty}, "
        return f"{ISetIfAll.op_name()}({config}{self.cop}, {self.test})"
